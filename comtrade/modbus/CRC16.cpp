...
UINT16 CRC16_Create(unsigned char *puchMsg, UINT16 usDataLen)
{
	...
	{    
		uIndex = uchCRCHi^((*puchMsg)&0x00ff); //calculate the CRC
...
UINT16 CRC16_CreateInit(unsigned char *puchMsg, UINT16 usDataLen,UINT16 iCRCInit)
        {
				 unsigned char uchCRCHi =iCRCInit>>8 ; // high CRC byte initialized
	       unsigned char uchCRCLo =iCRCInit&0xFF ; // low CRC byte initialized
				 UINT16 uIndex ; // will index into CRC lookup
	       while (usDataLen--) // pass through message buffer
	        {   
		       uIndex = uchCRCHi^((*puchMsg)&0x00ff); //calculate the CRC
...
{  
	UINT16 n,crcresult;
	usDataLen-=2;
	crcresult=CRC16_Create(puchMsg,usDataLen);
	n=puchMsg[usDataLen];
	n<<=8;
	usDataLen++;
	n|=puchMsg[usDataLen];
